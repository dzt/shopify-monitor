        ...
        if (tryParseJSON(body)) {
            ...
            for (var i = 0; i < jsonBodyProduct.product.variants.length; i++) {
              if (finalStock === 'Unavailable') {
                var str = `<http://${parseUrl(url).resource}/cart/${jsonBodyProduct.product.variants[i].id}:1|${jsonBodyProduct.product.variants[i].option1}>`
              } else {
                var str = `<http://${parseUrl(url).resource}/cart/${jsonBodyProduct.product.variants[i].id}:1|${jsonBodyProduct.product.variants[i].option1} / Stock: ${jsonBodyProduct.product.variants[i].inventory_quantity}>`
              }
              data.push(str)
            }
